...
class AdminPostController extends AdminbaseController {
	...
	function _initialize() {
		parent::_initialize();
		$this->posts_obj = D("Docs");
		$this->terms_obj = D("Docterms");
		$this->terms_relationship = D("DoctermRelationships");
	}
	function index(){
		$this->_lists();
		$this->_getTree();
	...
	public function edit(){
		...
		$term_id = intval(I("get.term"));
		if(empty($term_id)){
			$term_id = M('DoctermRelationships')->where("object_id=$id")->getField('term_id');
		}
		$term=$this->terms_obj->where("term_id=$term_id")->find();
	...
	private  function _lists($status=1){
		...
		->alias("a")
		->join(C ( 'DB_PREFIX' )."docs b ON a.object_id = b.id")
		->where($where)
		->count();
	...
	private function _getTree(){
		...
		$tree->nbsp = '&nbsp;&nbsp;&nbsp;';
		foreach ($result as $r) {
			$r['str_manage'] = '<a href="' . U("AdminDocterm/add", array("parent" => $r['term_id'])) . '">添加子类</a> | <a href="' . U("AdminDocterm/edit", array("id" => $r['term_id'])) . '">修改</a> | <a class="J_ajax_del" href="' . U("AdminDocterm/delete", array("id" => $r['term_id'])) . '">删除</a> ';
			$r['visit'] = "<a href='#'>访问</a>";
